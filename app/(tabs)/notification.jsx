...
const Notification = () => {
  ...
  const markAsUnread = async (notificationId) => {
    try {
      await axios.put(
        `${API_URL}/notifications/notifications-user/${notificationId}/unread`,
        null,
        {
          headers: {
            "X-API-Key": AUTH_KEY,
  ...
  const markAllAsRead = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/notifications/notifications-all/${user.id}/read-all`,
        null,
        {
          headers: {
            "X-API-Key": AUTH_KEY,
  ...
  const markAllAsUnread = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/notifications/notifications-all/${user.id}/unread-all`,
        null,
        {
          headers: {
            "X-API-Key": AUTH_KEY,
  ...
  const deleteNotification = async (notificationId) => {
    try {
      ...
      setNotifications(
        notifications.filter((note) => note.id !== notificationId)
      );
  ...
  const clearAllNotifications = async () => {
    ...
    try {
      await axios.delete(
        `${API_URL}/notifications/delete-all/${user.id}/clear`,
        {
          headers: {
            "X-API-Key": AUTH_KEY,
          },
      ...
      >
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)" }}>
          <View className="flex-1 justify-end">
            <Surface
              className="bg-white rounded-t-3xl overflow-hidden elevation-5"
              style={{ maxHeight: height * 0.85 }}
            >
              {/* Colorful header based on notification type */}
              <View
                style={{ backgroundColor: getModalHeaderColor(), padding: 16 }}
              >
              ...
              >
                {/* Title */}
                <Text className="font-psemibold text-xl mt-2 mb-3">
          ...
          <View className="flex-row items-center justify-between w-full mb-3">
            ...
            <View className="flex-row">
              ...
              <Menu
                ...
                anchor={
                  <IconButton icon="check-all" onPress={toggleMarkAllMenu} />
                }
